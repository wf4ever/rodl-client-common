...
public class NotificationService implements Serializable {
	...
	void init() {
		try {
			...
			this.notificationsUriTemplateString = serviceResource
					.listProperties(pl.psnc.dl.wf4ever.vocabulary.NotificationService.notifications)
	 ...
	 *            the research object URI. If set, only notifications related to
	 *            this RO will be returned.
	 * @param from
	 *            the timestamp of the oldest notification that should be
	 *            returned.
	 * @param to
	 *            the timestamp of the most recent notification that should be
	 *            returned.
	 ...
	 * @return the URI for the notifications feed
	 */
	public URI getNotificationsUri(URI researchObjectUri, DateTime from, DateTime to, URI source,
			Integer limit) {
		...
		if (from != null) {
			uriTemplate = uriTemplate.set("from",
					ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC).print(from));
		}
		if (to != null) {
			uriTemplate = uriTemplate.set("to",
					ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC).print(to));
		}
		if (source != null) {
	 ...
	 *             when the feed is invalid
	 */
	public ArrayList<Notification> getNotifications(URI researchObjectUri, DateTime from,
			DateTime to) throws NotificationsException {
